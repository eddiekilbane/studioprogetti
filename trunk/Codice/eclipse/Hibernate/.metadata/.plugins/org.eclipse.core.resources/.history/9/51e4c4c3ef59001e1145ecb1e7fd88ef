package demo.associations.ManyToMany.bidirectional.test;
...
import demo.associations.ManyToMany.bidirectional.beans.*;
...
//			2) RENDO PERSISTENTE L'OGGETTO User
			...
			System.out.println("PRIMO INDIRIZZO:inserito con ID: "+nuovoIndirizzo.getId());

			sessione.save(nuovoIndirizzo2);
			System.out.println("SECONDO INDIRIZZO: inserito con ID: "+nuovoIndirizzo2.getId());
			...
			sessione.getTransaction().commit();
			 ...
			 //...ora gli indirizzi di nuovoUtente..:
	        testRelazioni(nuovoUtente);
